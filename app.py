...
def page_daily_input():
    ...
    c3,c4=st.columns(2)
    _warn=st.session_state.get("show_missing_warning",False)
    _m_label="🔴 관리자 * (필수 입력)" if _warn and not di.get("manager","") else "관리자 *"
    manager=c3.text_input(_m_label,value=di.get("manager",""),placeholder="예: 김성균",key="inp_manager")
    _l_label="🔴 작업 위치 * (필수 입력)" if _warn and not di.get("location","") else "작업 위치 *"
    ...
    _w_label="🔴 투입 인원 현황 (공종별) * (필수 입력)" if _warn and not di.get("workers","") else "투입 인원 현황 (공종별) *"
    workers=st.text_area(_w_label,value=di.get("workers",""),key="inp_workers",
                          placeholder="예: 철근공 10명, 형틀공 5명",height=65)
    _wp_label="🔴 주요 작업 내용 * (필수 입력)" if _warn and not di.get("work_process","") else "주요 작업 내용 *"
